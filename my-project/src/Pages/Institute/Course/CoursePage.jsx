...
const CoursePage = ({ theme = "light" }) => {
  ...
  const courseDetails = {
    ...
    description:
      "A comprehensive bootcamp covering React.js from basics to advanced concepts.",
    instructor: "John Doe",
  ...
  const courseId = "6802573f18639d3478cbb50c";
  ...
  const handleAddModule = async () => {
    if (newModuleTitle && newModuleDuration) {
      ...
      await dispatch(addModule(data))
        .unwrap()
        .then((res) => {
          console.log("Module added successfully:", res);
        })
        .catch((error) => {
  ...
  return (
    <div className={`${bg} min-h-screen p-8`}>
      ...
      >
        <h1 className={`${primary} text-4xl font-extrabold mb-4`}>
          {courseDetails.title}
        </h1>
      ...
      >
        ...
        <ul className="space-y-6">
          {modules.map((module, index) => (
            <li
              ...
              } p-6 rounded-xl shadow-lg hover:${
        ...
        <button
          ...
          } w-full py-3 px-6 text-white rounded-lg hover:opacity-80 transition duration-300 flex items-center justify-center`}
        >
      ...
      {showAddModuleForm && (
        ...
        >
          ...
          >
            <h3 className={`${primary} text-3xl font-semibold mb-6`}>
              Add New Module
            </h3>
            <div className="mb-4">
            ...
            <button
              ...
              className={`w-full py-3 px-6 ${
                isDark ? "bg-darkSecondary" : "bg-lightSecondary"
